...
export const AddOrganizationModal: FunctionComponent<AddOrganizationModalPropsInterface> = (
    ...
    function setAutoGeneratedOrgHandle(sanitizedValue: string) {
        ...
        const baseOrgHandle: string = isOrgHandleDotExtensionMandatory
            ? sanitizedValue + OrganizationManagementConstants.SAMPLE_ORG_HANDLE_DOMAIN_EXTENSION
            : sanitizedValue;

        checkOrgHandleAvailability({ orgHandle: baseOrgHandle })
            .then((response: CheckOrgHandleResponseInterface) => {
                if (response.available) {
                    // The sanitized value is available, use it as the organization handle.
                    setOrgHandleError("");
                    setOrgHandle(baseOrgHandle);
                } else {
                    // The sanitized value is not available, append random characters and generate the handle.
