...
/**
 * Single Email address field form component for My Account user profile.
 */
const SingleEmailFieldForm: FunctionComponent<SingleEmailFieldFormPropsInterface> = ({
    fieldSchema: schema,
    fieldLabel,
    initialValue,
    pendingEmailAddress,
    ...
    const renderPendingVerificationIcon= (): ReactElement => {
        return (
            <Popup
                ...
                header={
                    t("myAccount:components.profile.messages.emailConfirmation.header")
                }
                inverted
            />
        );
        ...
        return (
            <List.Content>
                <List.Description className="with-max-length">
                    { fieldValue }
                    { (fieldValue === pendingEmailAddress) && renderPendingVerificationIcon() }
                </List.Description>
            </List.Content>
        );
    };
...
export default SingleEmailFieldForm;
